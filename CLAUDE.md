...
- **Vínculo = aceite explícito** (migration `20260925180000`): `family_group_members.status` pending|active (CHECK active ⇔ `user_id`), token de 64 hex com só o SHA-256 no banco (`invite_token_hash`, sem GRANT de SELECT para o cliente), validade 7 dias. Convidar/reenviar = Edge Function `family-invite` (gate strict, `orbi_family_invite_issue` via `userClient`, e-mail pelo Resend em `_shared/email.ts`, secrets `RESEND_API_KEY`/`EMAIL_FROM`, layout de `templates/confirmation.html`); reenviar troca o token. Cliente não faz INSERT/UPDATE na tabela. `/invite/accept` (`pages/InviteAccept.tsx`): `orbi_family_invite_preview` → "Aceitar convite" → `orbi_family_invite_accept` (conta do e-mail convidado, e-mail confirmado, fora de outro grupo, dono com `familia_compartilhada`, revalidado sob lock) → cena `OrbitUnion` (CSS puro, `components/family/orbit.css`). Sem sessão, o token fica em localStorage (`lib/family-invite.ts`) e `resolvePostAuthRoute` volta para o convite antes de planos/cobrança. Não existe mais vínculo por igualdade de e-mail (`orbi_claim_family_invites` removida). Foto de quem convidou na revisão: preview devolve `inviter_avatar_path` (só `ready`) e `orbi_can_read_avatar` libera o bucket via `orbi_is_pending_inviter` (convite pendente, não expirado, para o e-mail confirmado do leitor) — migration `20260925200000`.
