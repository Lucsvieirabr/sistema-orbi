DROP FUNCTION IF EXISTS public.orbi_budget_overview(date, text);
DROP FUNCTION IF EXISTS public.orbi_monthly_closing(date, text);

CREATE OR REPLACE FUNCTION public.orbi_budget_overview(
...
REVOKE ALL ON FUNCTION public.orbi_budget_overview(date, text, boolean) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.orbi_monthly_closing(date, text, boolean) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.orbi_budget_overview(date, text, boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.orbi_monthly_closing(date, text, boolean) TO authenticated;

COMMENT ON FUNCTION public.orbi_budget_overview(date, text, boolean) IS
  'Orçamentos do mês com consumo, agendado, média de 3 meses e sugestões. p_exclude_projects (padrão true) tira gastos de Projetos de Vida do consumo. Exige feature orcamentos.';
COMMENT ON FUNCTION public.orbi_monthly_closing(date, text, boolean) IS
  'DRE pessoal do mês. p_exclude_projects (padrão true) isola lançamentos de Projetos de Vida: saem das linhas e da tendência e aparecem no bloco projects. Exige feature dre_pessoal.';
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    PERFORM cron.unschedule(jobid) FROM cron.job WHERE jobname IN ('orbi-projects-auto-archive', 'orbi-personal-inflation-monthly');
    PERFORM cron.schedule('orbi-projects-auto-archive', '10 3 * * *', 'SELECT public.orbi_projects_auto_archive()');
    PERFORM cron.schedule('orbi-personal-inflation-monthly', '30 4 1 * *', 'SELECT public.orbi_personal_inflation_run_monthly()');
  END IF;
END
$cron$;
